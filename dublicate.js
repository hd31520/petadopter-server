...
// IMPORTANT: On Vercel, these variables MUST be set in the Vercel project settings (Environment Variables).
...
const jwt = require('jsonwebtoken'); // Assuming you have 'jsonwebtoken' installed and use it for JWT.
...
try {
    // CRITICAL CHECK: Ensure FB_SERVICE_KEY is provided.
    if (!process.env.FB_SERVICE_KEY) {
        // Log error and exit process if Firebase key is missing.
        // Vercel will show this as a deployment failure, which is desired.
        console.error('CRITICAL ERROR: FB_SERVICE_KEY environment variable is not set. Firebase Admin SDK cannot be initialized.');
        process.exit(1);
    }

    // Decode the Base64 service account key.
    // Ensure the FB_SERVICE_KEY on Vercel is a single, correctly Base64 encoded string.
    const serviceAccountBase64 = process.env.FB_SERVICE_KEY;
    ...
    console.error('CRITICAL ERROR: Failed to initialize Firebase Admin SDK. Please check FB_SERVICE_KEY in Vercel Environment Variables and its Base64 encoding:', error.message);
    process.exit(1); // Exit if Firebase initialization fails critically
}

// --- MongoDB Connection Setup ---
const uri = process.env.URI;
// CRITICAL CHECK: Ensure MongoDB URI is provided.
if (!uri) {
    console.error('CRITICAL ERROR: MONGODB_URI environment variable is not defined! MongoDB connection cannot be established.');
    process.exit(1); // Exit if MongoDB URI is not set, as the app cannot function without it.
}
...
async function findDocumentById(collection, id) {
    if (!collection) {
        console.error(`findDocumentById called with undefined collection.`);
        return null;
    }
    let document = null;
...
// This function will handle the MongoDB connection and collection assignments.
// It's called once when the server starts.
async function connectToDatabase() {
    try {
        // CRITICAL: UNCOMMENTED AND AWAITED!
        // await client.connect();
        db = client.db("adopty"); // Assign to the global db variable

        // Assign collections after successful connection
        usersCollection = db.collection("users");
        petsCollection = db.collection("pets");
        donationCamCollection = db.collection("donation-cam"); // Corrected collection name if it was "donationCam"
        ...
        // For Vercel, if MongoDB connection fails, the app is likely unusable.
        // We'll exit here to ensure Vercel marks the deployment as failed.
        process.exit(1);
        return false; // Indicate failed connection (though process.exit will prevent return)
    }
}

// --- Middleware for verifying Firebase ID Token ---
// This middleware now checks if Firebase was successfully initialized.
const verifyFBToken = async (req, res, next) => {
    ...
    try {
        ...
        req.decoded = decodedToken; // Attach decoded token to request
        next();
    } catch (error) {
...
// --- Middleware to attach user role from DB ---
// This middleware now checks if MongoDB was successfully connected.
const attachUserRole = async (req, res, next) => {
    // Ensure collections are ready before attempting to query
    if (!db || !usersCollection) {
        console.error('Database or usersCollection not ready in attachUserRole middleware.');
        return res.status(500).send({ message: 'Server configuration error: Database not connected or collections not initialized.' });
    }
    if (!req.decoded || !req.decoded.email || !req.decoded.uid) {
        return res.status(401).send({ message: 'Unauthorized: User email or UID not found in token.' });
    }
    try {
        ...
        if (user) {
            ...
            req.decoded.role = 'user'; // Default role if not found
            // Optionally, create a basic user entry if authenticated via Firebase but not in DB
        ...
        res.status(500).send({ message: 'Internal server error during role attachment.' });
    }
};
...
// --- Middleware for checking volunteer role ---
const verifyVolunteer = (req, res, next) => {
    if (!req.decoded || (req.decoded.role !== 'volunteer' && req.decoded.role !== 'admin')) {
        return res.status(403).send({ message: 'Forbidden: Volunteer or Admin access required.' });
    }
    next();
};
...
// JWT Token generation endpoint (example, ensure 'jsonwebtoken' is installed)
...
app.post("/users", async (req, res) => {
    ...
    if (!usersCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
    const user = req.body;
...
app.patch(
    ...
    async (req, res) => {
        ...
        try {
            const userToUpdate = await findDocumentById(usersCollection, userId);
            if (!userToUpdate) {
...
app.delete(
    ...
    async (req, res) => {
        ...
        try {
            const userToDelete = await findDocumentById(usersCollection, userId);
            if (!userToDelete) {
                return res.status(404).send({ message: "User not found." });
            }
...
app.get("/donation-cam", async (req, res) => {
    if (!donationCamCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
    try {
...
app.post(
    ...
    attachUserRole,
    async (req, res) => {
        if (!donationsCollection || !donationCamCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
...
app.get(
    ...
    attachUserRole,
    async (req, res) => {
        if (!donationsCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
        const requestedUserId = req.params.userId;
        const authUserId = req.decoded.uid;
...
app.post(
    ...
    verifyVolunteer,
    async (req, res) => {
        if (!tasksCollection || !usersCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
...
app.get("/my-tasks", verifyFBToken, attachUserRole, async (req, res) => {
    if (!tasksCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
    try {
        const tasks = await tasksCollection
            .aggregate([
                {
                    $match: { assignedTo: req.decoded.uid },
                },
                {
                    $lookup: {
                        from: "users",
...
app.get(
    ...
    verifyAdmin,
    async (req, res) => {
        if (!adoptionRequestsCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
        try {
...
app.get(
    ...
    attachUserRole,
    async (req, res) => {
        if (!petsCollection || !adoptionRequestsCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
...
app.patch(
    ...
    attachUserRole,
    async (req, res) => {
        if (!adoptionRequestsCollection || !petsCollection) { return res.status(500).send({ message: 'Database not ready.' }); }
...
// This function encapsulates the database connection and then starts the Express server.
async function startServer() {
    const isDbConnected = await connectToDatabase(); // Attempt to connect to DB
    if (isDbConnected) {
        // Only start listening for requests if DB connection was successful
        app.listen(port, () => {
            console.log(`Adopty Backend is running on port ${port}`);
        });
    } else {
        console.error("Server could not start due to database connection failure.");
        // process.exit(1) is already called in connectToDatabase if it fails,
        // so this else block might not be reached in practice if the exit happens.
    }
}

// Call the function to start the server. This is the entry point for your application logic.
startServer();

// --- Global Error Handling Middleware ---
// This catches any unhandled errors that occur in your Express routes.
